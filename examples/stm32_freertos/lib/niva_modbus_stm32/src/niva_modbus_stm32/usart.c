...
inline void modbus_usart_start_receive(uint8_t* data, size_t len) {
    ...
    modbus_dma_config_transmit(MODBUS_USART_DMA_RX, MODBUS_USART_DMA_RX_CH_NUM, (uint32_t)&MODBUS_USART_RDR, (uint32_t)data, (uint32_t)len);
    DMA_RECEIVE_LEN = len;

    LL_USART_EnableDMAReq_RX(MODBUS_USART);
    LL_USART_EnableIT_RXNE(MODBUS_USART);
    LL_USART_EnableDirectionRx(MODBUS_USART);
    modbus_dma_enable(MODBUS_USART_DMA_RX, MODBUS_USART_DMA_RX_CH_NUM);
}

inline uint32_t modbus_usart_get_received_len() {
    return DMA_RECEIVE_LEN - modbus_dma_get_received_length(MODBUS_USART_DMA_RX, MODBUS_USART_DMA_RX_CH_NUM);
}
...
inline void modbus_usart_transmit(uint8_t* data, size_t len) {
    ...
    modbus_dma_config_transmit(MODBUS_USART_DMA_TX, MODBUS_USART_DMA_TX_CH_NUM, (uint32_t)&MODBUS_USART_TDR, (uint32_t)data, (uint32_t)len);

    LL_USART_EnableDMAReq_TX(MODBUS_USART);
    LL_USART_EnableDirectionTx(MODBUS_USART);
    LL_USART_EnableIT_TC(MODBUS_USART);
    modbus_dma_enable(MODBUS_USART_DMA_TX, MODBUS_USART_DMA_TX_CH_NUM);
}
